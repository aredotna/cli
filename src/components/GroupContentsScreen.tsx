...
export function GroupContentsScreen({
  ...
  const {
    ...
    error,
    isLoading: loading,
  } = useSWR(`session-group-contents:${slug}:${page}:${PER}`, () =>
    Promise.all([
      getData(
        client.GET("/v3/groups/{id}", { params: { path: { id: slug } } }),
      ),
      getData(
    ...
    ]).then(([group, contents]) => ({ group, contents })),
  );
  ...
  if (loading) return <Spinner label={`Loading group contents for ${slug}`} />;
  ...
  return (
    <Box flexDirection="column">
      <Box marginBottom={1}>
        <Text bold>{data.group.name}</Text>
        <Text dimColor>@{data.group.slug} · contents</Text>
      </Box>
